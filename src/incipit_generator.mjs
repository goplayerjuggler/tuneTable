...
import generateIncipit from './incipits.js';
...
async function processIncipits() {
  try {
    ...
    // Import the data
    const tunesModule = await import('./tunes.json.js');
      ...
      if (inTunesArray) {
        ...
        if (char === '{' && braceDepth === 0) {
          ...
          if (braceDepth === 0 && currentObjectStart !== -1) {
            ...
            const tuneContent = updatedContent.substring(currentObjectStart + 1, i);
            ...
            if (incipitsToAdd.has(currentTuneIndex)) {
              ...
              const needsComma = tuneContent.trim().length > 0 && !tuneContent.trim().endsWith(',');
